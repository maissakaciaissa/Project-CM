...
def hierarchical_search(bloc1, searchBox, pyramids_b, bloc_width, bloc_height, levels):
    ...
    for level in range(levels - 1, -1, -1):  # Traverse levels (low to high)
        ...
        # Search in a window 3*3 around the previous position
        for dy in range(-1, 2):
            for dx in range(-1, 2):
...
def process_selected_block(img1, img2, roi, levels, img_label_residual, img_label_reconstructed):
    ...
    if best_match is not None:
        ...
        reconstructed_block_display = np.clip(reconstructed_block, 0, 255).astype(np.uint8)
    ...
    reconstructed_image_color_rgb = cv2.cvtColor(reconstructed_block_display, cv2.COLOR_BGR2RGB)
